...
add_action('pre_get_posts', function($query) {
    if (!is_admin() && $query->is_main_query() && is_post_type_archive('du-an')) {
...
function render_document_table_and_pagination($term_id, $selected_year, $per_page, $current_page) {
    ...
    if (empty($term_id) || !is_numeric($term_id)) {
        ob_start();
        ?>
        <table class="detail-table h-10 w-full">
        <thead class="-lg:hidden">
            <tr class="border-b-2 border-primary-2">
                <th class="w-[calc(48/1241*100%)] heading-4 text-primary-2 p-1.5 "><?php _e('STT', 'canhcamtheme'); ?></th>
                <th class="w-[calc(841/1241*100%)] heading-4 text-primary-2 p-4 text-left"><?php _e('Nội dung công bố', 'canhcamtheme'); ?></th>
                <th class="w-[calc(160/1241*100%)] heading-4 text-primary-2 px-2.5 py-4 text-left"><?php _e('Ngày đăng tải', 'canhcamtheme'); ?></th>
                <th class="w-[calc(160/1241*100%)] px-2.5 py-4"></th>
            </tr>
        </thead>
        <tbody>
            <tr>
                <td colspan="4" style="text-align:center;"><?php esc_html_e('Không có tài liệu nào.', 'canhcamtheme'); ?></td>
            </tr>
        </tbody>
        </table>
        <?php
    ...
    <tbody>
        <?php if ($paged_query->have_posts()) : ?>
            ...
            ?>
                <tr class="row-tb -lg:grid -lg:grid-cols-8 border-b border-utility-gray-100 last:border-none group">
                    <td class="px-2.5 py-4 transition-300 group-hover:text-primary-2 text-body-1 text-center -lg:col-span-1 -lg:text-left -lg:font-medium -lg:flex-center"><?php echo str_pad($start + $index + 1, 2, '0', STR_PAD_LEFT); ?></td>
                    <td class="text-body-1 -lg:px-2.5 p-4 -lg:col-span-7"> 
                        <?php if ($file_url): ?>
                            <a class="transition-300 group-hover:text-primary-2 line-clamp-2" href="<?php echo esc_url($file_url); ?>" target="_blank" rel="noopener" download><?php echo esc_html($file_title ? $file_title : 'Tài liệu'); ?></a>
                        <?php else: ?>
                            <a class="transition-300 group-hover:text-primary-2 line-clamp-2" href="<?php echo esc_url(get_permalink()); ?>"><?php echo esc_html($file_title); ?></a>
                        <?php endif; ?>
                    </td>
                    <td class="text-body-1 px-2.5 py-4 lg:text-center transition-300 group-hover:text-primary-2 -lg:col-start-2 -lg:col-end-5">
                        <?php echo esc_html($post_date); ?>
                    </td>
                    <td class="col-btn-down px-2.5 py-4 -lg:col-span-4">
                        <?php if ($file_url): ?>
                            <a class="document-dowload-btn flex-center h-full gap-x-3 text-utility-gray-500 cursor-pointer select-none" href="<?php echo esc_url($file_url); ?>" download target="_blank" rel="noopener">
                                <div class="block-icon text-base"><i class="fa-light fa-download"></i></div>
                                <div class="content text-body-1 font-normal"><?php _e('Tải về', 'canhcamtheme'); ?></div>
                            </a>
                        <?php else: ?>
                            <a class="document-dowload-btn flex-center h-full gap-x-3 text-utility-gray-500 cursor-pointer select-none" href="<?php echo esc_url(get_permalink()); ?>" target="_blank" rel="noopener">
                                <div class="block-icon text-base"><i class="fa-light fa-arrow-right"></i></div>
...
<div class="pagination-container mt-4">
    <ul class="pagination flex items-center justify-center">
        ...
        ?>
        <li class="pagination-item flex-center w-10 md:w-12 h-10 md:h-12 border-2 border-primary-2 rounded-1 heading-4 transition-300 hover:bg-primary-1 hover:border-primary-1 cursor-pointer prev">
            <a href="#" data-paged="<?php echo $prev_page; ?>"><i class="fa-regular fa-chevron-left"></i></a>
        </li>
        <?php endif; ?>
        ...
        for ($i = 1; $i <= $total_pages; $i++) {
            ...
            ?>
            <li class="pagination-item flex-center w-10 md:w-12 h-10 md:h-12 border-2 border-primary-2 rounded-1 heading-4 transition-300 hover:bg-primary-1 hover:border-primary-1 cursor-pointer <?php echo $active; ?>">
                <a href="#" data-paged="<?php echo $i; ?>"><?php echo $i; ?></a>
            </li>
            <?php
        }
        ...
        ?>
        <li class="pagination-item flex-center w-10 md:w-12 h-10 md:h-12 border-2 border-primary-2 rounded-1 heading-4 transition-300 hover:bg-primary-1 hover:border-primary-1 cursor-pointer next">
            <a href="#" data-paged="<?php echo $next_page; ?>"><i class="fa-regular fa-chevron-right"></i></a>
        </li>
        <?php endif; ?>
    </ul>
</div>
...
function load_more_careers() {
    ...
    if ($query->have_posts()) {
        while ($query->have_posts()) : $query->the_post();
            ...
            ?>
            <tr class="row-job -md:grid -md:grid-cols-2 -md:w-full text-body-1 border border-utility-gray-100 bg-secondary-1 bg-opacity-[0.05]">
                <td class="text-center -md:col-span-full -md:font-medium -md:text-left -md:p-2"><?php echo sprintf("%02d", $count); ?></td>
                <td class="p-2 px-4 py-2 md:py-3 -md:col-span-full"><a href="<?php the_permalink(); ?>"><?php the_title(); ?></a></td>
